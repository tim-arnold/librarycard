...
/**
 * ⚠️  NEW ISOLATED ACCOUNT STAGING DATA SEEDING SCRIPT ⚠️
 * 
 * 🚨 CRITICAL WARNING: This script DELETES ALL DATA and reseeds the database!
 * 🚨 ONLY for new isolated staging account - NEVER run against production!
 * 
 * This script seeds the new isolated staging database with comprehensive sample data:
  ...
  locations: [
    {
      ...
      description: 'Personal book collection for staging tests - programming, technical references, and fiction',
    ...
    },
    {
      ...
      description: 'Work reading collection for staging - management, business, and professional development',
      owner_id: 'staging-user-1',
      created_at: new Date().toISOString(),
      updated_at: new Date().toISOString()
    },
    {
      name: 'Staging Community Library',
      description: 'Shared collection for staging tests - diverse fiction and non-fiction',
      owner_id: 'staging-user-2',
      created_at: new Date().toISOString(),
      updated_at: new Date().toISOString()
    }
  ],
...
function execWrangler(command, description) {
  try {
    ...
    const result = execSync(command, { stdio: 'pipe', timeout: 30000, encoding: 'utf8' });
    return result;
  } catch (error) {
...
function seedUsers() {
  ...
  for (const user of seedData.users) {
    // Single-line SQL with proper boolean conversion
    const emailVerified = user.email_verified ? 1 : 0;
    const sql = `INSERT OR REPLACE INTO users (id, email, first_name, last_name, password_hash, user_role, email_verified, auth_provider, created_at, updated_at) VALUES ('${user.id}', '${user.email}', '${user.first_name}', '${user.last_name}', '${user.password_hash}', '${user.user_role}', ${emailVerified}, '${user.auth_provider}', '${user.created_at}', '${user.updated_at}');`;
    
    const tempFile = writeSqlFile(sql, `seed-user-${user.id}.sql`);
    execWrangler(`CLOUDFLARE_API_TOKEN=${process.env.CLOUDFLARE_API_TOKEN_STAGING_NEW} npx wrangler d1 execute librarycard-db-staging-new --config=wrangler.staging-new.toml --env staging --remote --file="${tempFile}"`, `Seeding user ${user.email}`);
...
function seedGenres() {
  ...
  const adminUserId = 'staging-user-3'; // superadmin@localhost
  ...
  execWrangler(`CLOUDFLARE_API_TOKEN=${process.env.CLOUDFLARE_API_TOKEN_STAGING_NEW} npx wrangler d1 execute librarycard-db-staging-new --config=wrangler.staging-new.toml --env staging --remote --file="${tempFile}"`, 'Seeding curated genres');
  fs.unlinkSync(tempFile);
...
function seedLocationsAndShelves() {
  ...
  for (const [locationKey, locationData] of Object.entries(seedData.bookCollections)) {
    ...
    const locationSql = `INSERT OR REPLACE INTO locations (id, name, description, owner_id, created_at, updated_at) VALUES (${locationId}, '${location.name}', '${location.description}', '${location.owner_id}', '${location.created_at}', '${location.updated_at}');`;
    
    const locationFile = writeSqlFile(locationSql, `seed-location-${locationId}.sql`);
    execWrangler(`CLOUDFLARE_API_TOKEN=${process.env.CLOUDFLARE_API_TOKEN_STAGING_NEW} npx wrangler d1 execute librarycard-db-staging-new --config=wrangler.staging-new.toml --env staging --remote --file="${locationFile}"`, `Seeding location ${location.name}`);
    fs.unlinkSync(locationFile);
    
    // Seed shelves for this location
    for (const [shelfKey, shelfData] of Object.entries(locationData)) {
      const shelfSql = `INSERT OR REPLACE INTO shelves (id, name, location_id, created_at, updated_at) VALUES (${shelfId}, '${shelfData.name}', ${locationId}, '${new Date().toISOString()}', '${new Date().toISOString()}');`;
      
      const shelfFile = writeSqlFile(shelfSql, `seed-shelf-${shelfId}.sql`);
      execWrangler(`CLOUDFLARE_API_TOKEN=${process.env.CLOUDFLARE_API_TOKEN_STAGING_NEW} npx wrangler d1 execute librarycard-db-staging-new --config=wrangler.staging-new.toml --env staging --remote --file="${shelfFile}"`, `Seeding shelf ${shelfData.name}`);
...
function seedLocationMemberships() {
  ...
  // Assign all users to all locations so they can access everything
  const memberships = [
    ...
    { user_id: 'staging-user-1', location_id: 3 }, // Admin user -> Community Library
    { user_id: 'staging-user-2', location_id: 1 }, // Test user -> Home Library
    { user_id: 'staging-user-2', location_id: 2 }, // Test user -> Office Library
    { user_id: 'staging-user-2', location_id: 3 }, // Test user -> Community Library
    ...
    { user_id: 'staging-user-3', location_id: 3 }  // Super admin -> Community Library
  ];
  
  const membershipSql = memberships.map(membership => 
    `INSERT OR IGNORE INTO location_members (user_id, location_id) VALUES ('${membership.user_id}', ${membership.location_id});`
  ).join('\n');
  
  const membershipFile = writeSqlFile(membershipSql, 'seed-memberships.sql');
  execWrangler(`CLOUDFLARE_API_TOKEN=${process.env.CLOUDFLARE_API_TOKEN_STAGING_NEW} npx wrangler d1 execute librarycard-db-staging-new --config=wrangler.staging-new.toml --env staging --remote --file="${membershipFile}"`, 'Seeding location memberships');
  fs.unlinkSync(membershipFile);
...
function seedBooks() {
  ...
  let bookCount = 0;
  let shelfId = 1;
  const userId = 'staging-user-1'; // Admin user for most books
  ...
  for (const [locationKey, locationData] of Object.entries(seedData.bookCollections)) {
    for (const [shelfKey, shelfData] of Object.entries(locationData)) {
      for (const book of shelfData.books) {
        ...
        const bookSql = `INSERT OR REPLACE INTO books (title, authors, isbn, description, thumbnail, categories, published_date, page_count, shelf_id, added_by, created_at) VALUES ('${escapedTitle}', '${escapedAuthors}', '${book.isbn}', '${escapedDescription}', '${book.thumbnail}', '${escapedCategories}', '${book.published_date}', ${book.page_count}, ${shelfId}, '${userId}', '${new Date().toISOString()}');`;
        
        const bookFile = writeSqlFile(bookSql, `seed-book-${bookCount}.sql`);
        try {
          execWranglerSilent(`CLOUDFLARE_API_TOKEN=${process.env.CLOUDFLARE_API_TOKEN_STAGING_NEW} npx wrangler d1 execute librarycard-db-staging-new --config=wrangler.staging-new.toml --env staging --remote --file="${bookFile}"`, `Seeding book ${book.title}`);
          bookCount++;
        } catch (error) {
...
function addSampleRatings() {
  ...
  const ratingUsers = ['staging-user-1', 'staging-user-2', 'staging-user-1'];
  const sampleReviews = [
  ...
  for (const bookId of bookIds) {
    ...
    for (let i = 0; i < numRatings; i++) {
      ...
      const ratingSql = `INSERT OR IGNORE INTO book_ratings (book_id, user_id, rating, review_text, created_at, updated_at) VALUES (${bookId}, '${userId}', ${rating}, ${reviewText ? `'${reviewText.replace(/'/g, "''")}'` : 'NULL'}, '${new Date().toISOString()}', '${new Date().toISOString()}');`;
      
      const ratingFile = writeSqlFile(ratingSql, `seed-rating-${ratingCount}.sql`);
      try {
        execWranglerSilent(`CLOUDFLARE_API_TOKEN=${process.env.CLOUDFLARE_API_TOKEN_STAGING_NEW} npx wrangler d1 execute librarycard-db-staging-new --config=wrangler.staging-new.toml --env staging --remote --file="${ratingFile}"`, `Adding rating for book ${bookId}`);
        ratingCount++;
      } catch (error) {
...
function clearDatabase() {
  ...
  // Order matters due to foreign key constraints - use PRAGMA to disable temporarily
  const tables = [
    'book_ratings',
    'book_genres', 
    'genre_suggestions',
  ...
  // Create a comprehensive clearing script with foreign key handling
  const clearingSql = [
    'PRAGMA foreign_keys = OFF;',
    ...tables.map(table => `DELETE FROM ${table};`),
    'DELETE FROM sqlite_sequence WHERE name IN (\"books\", \"shelves\", \"locations\", \"curated_genres\", \"users\", \"book_ratings\", \"book_genres\", \"genre_suggestions\");',
    'PRAGMA foreign_keys = ON;'
  ].join('\n');
  
  const clearingFile = writeSqlFile(clearingSql, 'clear-all-data.sql');
  
  try {
    execWrangler(`CLOUDFLARE_API_TOKEN=${process.env.CLOUDFLARE_API_TOKEN_STAGING_NEW} npx wrangler d1 execute librarycard-db-staging-new --config=wrangler.staging-new.toml --env staging --remote --file="${clearingFile}"`, 'Clearing all data and resetting sequences');
    fs.unlinkSync(clearingFile);
  ...
  for (const { name, query } of queries) {
    try {
      ...
      const result = execWranglerSilent(
        `CLOUDFLARE_API_TOKEN=${process.env.CLOUDFLARE_API_TOKEN_STAGING_NEW} npx wrangler d1 execute librarycard-db-staging-new --config=wrangler.staging-new.toml --env staging --remote --command "${query}"`,
        `Verifying ${name}`
      );
...
function main() {
  ...
  try {
    ...
    log(`   • 3 sample users (admin, user, super admin)`);
    ...
    log(`   Admin: adminuser@staging.localhost / Admin123!`);
    log(`   User: testuser@staging.localhost / Test123!`);
    log(`   Super Admin: superadmin@staging.localhost / Super123!`);
