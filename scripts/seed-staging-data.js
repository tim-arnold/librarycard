 ...
 * 
 * This script seeds the staging development database with comprehensive sample data:
...
function seedLocationsAndShelves() {
  ...
  for (const [locationKey, locationData] of Object.entries(seedData.bookCollections)) {
    ...
    execWrangler(`npx wrangler d1 execute librarycard-db-staging --env staging --remote --file="${locationFile}"`, `Seeding location ${location.name}`);
    ...
    for (const [shelfKey, shelfData] of Object.entries(locationData)) {
      ...
      execWrangler(`npx wrangler d1 execute librarycard-db-staging --env staging --remote --file="${shelfFile}"`, `Seeding shelf ${shelfData.name}`);
...
function seedLocationMemberships() {
  ...
  execWrangler(`npx wrangler d1 execute librarycard-db-staging --env staging --remote --file="${membershipFile}"`, 'Seeding location memberships');
  ...
  for (const [locationKey, locationData] of Object.entries(seedData.bookCollections)) {
    for (const [shelfKey, shelfData] of Object.entries(locationData)) {
      for (const book of shelfData.books) {
        ...
        try {
          execWranglerSilent(`npx wrangler d1 execute librarycard-db-staging --env staging --remote --file="${bookFile}"`, `Seeding book ${book.title}`);
          bookCount++;
        } catch (error) {
...
function addSampleRatings() {
  ...
  for (const bookId of bookIds) {
    ...
    for (let i = 0; i < numRatings; i++) {
      ...
      try {
        execWranglerSilent(`npx wrangler d1 execute librarycard-db-staging --env staging --remote --file="${ratingFile}"`, `Adding rating for book ${bookId}`);
        ratingCount++;
      } catch (error) {
...
function clearDatabase() {
  ...
  try {
    execWrangler(`npx wrangler d1 execute librarycard-db-staging --env staging --remote --file="${clearingFile}"`, 'Clearing all data and resetting sequences');
  ...
  for (const { name, query } of queries) {
    try {
      ...
      const result = execWranglerSilent(
        `npx wrangler d1 execute librarycard-db-staging --env staging --remote --command "${query}"`,
        `Verifying ${name}`
      );
